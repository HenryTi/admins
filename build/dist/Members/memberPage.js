...
import { List, LMR, FA, Media, Muted, PropGrid } from 'tonva';
...
let MemberPage = class MemberPage extends React.Component {
    constructor() {
        super(...arguments);
        this.renderMemberRole = (role) => {
            let { name, discription } = role;
            return React.createElement(LMR, { className: "py-2 px-3 align-items-center", left: name, right: React.createElement(Muted, null, discription) });
        };
        this.setRole = () => {
    ...
    render() {
        ...
        let rows = [
            ...
            {
                type: 'component',
                component: React.createElement(Media, { icon: icon || appIcon, main: roleUser.assigned || nick || name, discription: disp })
            },
            '',
            {
                label: '备注名',
        ...
        let right = React.createElement("button", { className: "btn btn-success btn-sm", onClick: this.setRole }, "\u4FEE\u6539\u89D2\u8272");
...
let SetRole = class SetRole extends React.Component {
    ...
    renderRole(role, index) {
        return React.createElement(LMR, { className: "py-2 px-3 align-items-center", left: role.name, right: React.createElement("small", { className: "text-muted" }, role.discription) });
    }
    ...
    render() {
        let right = React.createElement("button", { className: "btn btn-success btn-sm", onClick: this.submit }, "\u4FDD\u5B58");
