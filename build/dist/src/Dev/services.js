...
import { PropGrid, Media } from 'tonva';
...
class Info extends React.Component {
    render() {
        ...
        if (app !== undefined)
            ...
            obj = { type: 'component', label: 'UQ', component: React.createElement("div", { className: "py-2" },
                    React.createElement(UqSpan, { id: uq, isLink: true })) };
        let rows = [
            '',
...
const idPickUqProps = {
    ...
    candidateItems: (params, key) => __awaiter(this, void 0, void 0, function* () {
        yield store.dev.searchUq.first(key);
        return store.dev.searchUq.items;
    }),
    moreCandidates: () => __awaiter(this, void 0, void 0, function* () {
        yield store.dev.searchUq.more();
    }),
    row: (item, index) => {
        return React.createElement("div", null, item.name + ' ' + item.discription);
    },
            ...
            formRows: [
                ...
                serverRow,
                {
                    ...
                    face: {
                        type: 'pick-id',
                        initCaption: '请选择UQ',
                        pick: createIdPick(idPickUqProps),
                        fromPickedItem: (item) => {
                            return { id: item.id, caption: item.name + ' ' + item.discription };
                        },
                    },
