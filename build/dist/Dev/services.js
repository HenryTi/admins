...
class Info extends React.Component {
    render() {
        ...
        let rows = [
            '',
            { type: 'component', component: React.createElement(Media, { icon: consts.appIcon, main: discription, discription: url }) },
            '',
...
const idPickServerProps = {
    ...
    row: (item, index) => {
        return React.createElement("div", null, item.discription + ' ' + item.cloud + ' ' + item.ip);
    },
};
...
const servicesProps = {
    ...
    row: (item) => {
        return React.createElement(Row, { icon: consts.appItemIcon, main: item.url, vice: serviceTypeNames[item.type] + ': ' + item.discription });
    },
