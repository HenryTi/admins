...
import { Page, appInFrame } from 'tonva';
import { List, Muted, LMR, FA } from 'tonva';
import { VOpBinding } from './vOpBinding';
import { centerApi, entityIcons, ControllerUq } from 'tonva';
...
export class COpBinding extends ControllerUq {
    constructor() {
        ...
        this.appsView = () => React.createElement(Page, { header: this.label },
            React.createElement("div", { className: "text-muted p-3 small" },
                ...
                React.createElement("ol", { className: "pl-3", style: { listStyleType: '1', lineHeight: '1.8' } },
                    ...
                    React.createElement("li", null, "\u6BCF\u4E2AAPP\u4F1A\u4F7F\u7528\u4E00\u4E2A\u4EE5\u4E0A\u7684UQ\u3002UQ\u91CC\u9762\u5B9A\u4E49\u4E86Tuid\uFF0CAction\uFF0CMap\u548CSheet\u7B49\u5BF9\u8C61\u3002"),
        ...
        this.entityClick = (block, entity) => __awaiter(this, void 0, void 0, function* () {
            ...
            let ret = yield entityPosts.entity.query({ uq: entity.uq.id, entityName: entity.name });
            let opTos = {};
        ...
        this.sheetClickOld = (sheet) => __awaiter(this, void 0, void 0, function* () {
            let entityPosts = this.cUq.cFromName('query', 'getEntityPost');
            let ret = yield entityPosts.entity.query({ uq: sheet.uq.id, entityName: sheet.name });
            let opTos = {};
            for (let row of ret.ret) {
                let { op, post, team, section } = row;
                let opTo = opTos[op];
        ...
        this.uqRender = (uq, index) => {
            ...
            return React.createElement("div", { key: name, className: "my-2" },
                ...
                React.createElement(List, { className: "mt-3", items: settings, item: {
                        ...
                        onClick: (item) => this.settingClick(item, uq)
                    } }),
                blocks.map(block => {
        ...
        this.appView = (app) => React.createElement(Page, { header: app.name + '操作权限' }, app.uqs.map((v, index) => this.uqRender(v, index)));
        /*
    ...
    buildAppsUqs() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let ret = yield centerApi.get('/unit/apps-uqs', { unit: unit });
            this.apps = ret[0];
            let uqs = ret[1];
    ...
    setUqEntities(uq) {
        ...
        for (let i = 0; i < len;) {
            switch (lns[i]) {
                case 'tuid':
                    p = this.setEntities(uq.tuids = [], lns, i, uq);
                    break;
                case 'map':
                    p = this.setEntities(uq.maps = [], lns, i, uq);
                    break;
                case 'book':
                    p = this.setEntities(uq.books = [], lns, i, uq);
                    break;
                case 'history':
                    p = this.setEntities(uq.histories = [], lns, i, uq);
                    break;
                case 'pending':
                    p = this.setEntities(uq.pendings = [], lns, i, uq);
                    break;
                case 'query':
                    p = this.setEntities(uq.queries = [], lns, i, uq);
                    break;
                case 'action':
                    p = this.setEntities(uq.actions = [], lns, i, uq);
                    break;
                case 'sheet':
                    p = this.setSheets(uq.sheets = [], lns, i, uq);
                    break;
                default:
    ...
    setEntities(entities, lines, p, uq) {
    ...
    callSearchUser(uq) {
        return __awaiter(this, void 0, void 0, function* () {
            let user = yield this.vCall(VSearchUser, appInFrame.unit);
            return user;
        });
    }
